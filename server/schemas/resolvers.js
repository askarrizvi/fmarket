...
const resolvers = {
  ...
  Mutation: {
    ...
    // addProduct: async (parent, { product, description }, context) => {
    //   if (context.user) {
    //     const stall = await Product.create({ product, description });
    //     console.log(context.user._id);

    //     await User.findByIdAndUpdate(context.user._id, { $push: { products: product }}, { new: true }, function (err, result) {
    //       console.log("error:" + err);
    //       console.log("result:" + result);
    //     });
